...
void testList ( int testSize ) {
	...
	PRINT ( La );
	int i = La.size();
	while ( 0 < --i ) { La[i-1] += La[i]; La[i-1] >>= 1; } 
	PRINT ( La );
	...
	printf ( "\n  ==== Test %2d. FIND in\n", testID++ );
	PRINT ( Le );
	for ( int i = 0; i <= testSize * 2; i++ ) { //逐一测试[0,2n)中所有的可能
...
int main ( int argc, char* argv[] ) {
	...
	testList<int> ( atoi ( argv[1] ) ); //The elements in the list could be changed here.
	return 0;
}