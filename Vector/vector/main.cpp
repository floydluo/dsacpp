/************
* For Test and Generate a Program of Vector
*************/
...
int testID = 0; // Test ID

/******************************************************************************************
 * 测试：无序向量的（顺序）查找 Test: Unordered Vector (Search) Find
 ******************************************************************************************/
...
void TestFind ( Vector<T> & V, int n ) {
   ...
   for ( int i = 0; i <= V.size(); i++ ) { //一次查找每对相邻元素的均值，可能成果
...
void TestSearch ( Vector<T> & V ) {
   for ( int i = 0; i < V.size(); i++ ) { //依次查找向量中元素，当然成功
      T e =  V[i]; 
	  printf ( "Looking for" ); print ( e ); printf ( " in ...\n" ); print ( V );
      Rank r = V.search ( e );
      if ( V[r] == e ) 
		  printf ( "found at rank V[%d] = %d", r, V[r] );
      else 
...
void   testVector ( int testSize ) {
   ...
   for ( int i = 0; i < testSize; i++ ) 
	   V.insert ( dice ( i + 1 ), dice ( ( T ) testSize * 3 ) ); //在[0,3N)中选择n个数，随机插入
   PRINT ( V );
   permute ( V ); // What is this?
   PRINT(V);
   ...
   PRINT ( V );//This two lines state the aim of this test.
   ...
   /* For some reasons, there are no min function. How to call it?
   for ( int i = 0; i < V.size(); i += V.size() / 5 ) { 
